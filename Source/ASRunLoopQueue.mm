...
- (instancetype)initWithRunLoop:(CFRunLoopRef)runloop retainObjects:(BOOL)retainsObjects handler:(void (^)(id _Nullable, BOOL))handlerBlock
{
  if (self = [super init]) {
    ...
    _rootActivity = as_activity_create_verbose("Process run loop queue items", OS_ACTIVITY_NONE, OS_ACTIVITY_FLAG_DEFAULT);
    {
      // Log a message identifying this queue into the queue's root activity.
      as_activity_scope(_rootActivity);
...
- (void)processQueue
{
  ...
  if (count > 0) {
    as_activity_scope(as_activity_create_verbose("Process run loop queue batch", _rootActivity, OS_ACTIVITY_FLAG_DEFAULT));
