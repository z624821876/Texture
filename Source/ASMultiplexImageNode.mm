...
- (void)_loadImageIdentifiers
{
  ...
  as_log_debug(ASImageLoadingLog(), "%@ Best immediately available image identifier is %@", self, bestImmediatelyAvailableImageIdentifier);
...
- (void)_loadNextImage
{
  ...
  as_activity_scope(as_activity_create("Load next image for multiplex image node", OS_ACTIVITY_CURRENT, OS_ACTIVITY_FLAG_DEFAULT));
  as_log_debug(ASImageLoadingLog(), "Loading image for %@ ident: %@", self, nextImageIdentifier);
  ...
  if (_dataSourceFlags.image) {
    ...
    if (image) {
      as_log_debug(ASImageLoadingLog(), "Acquired image from data source for %@ ident: %@", self, nextImageIdentifier);
  ...
  if ([[nextImageURL scheme] isEqualToString:kAssetsLibraryURLScheme]) {
    ...
    [self _loadALAssetWithIdentifier:nextImageIdentifier URL:nextImageURL completion:^(UIImage *downloadedImage, NSError *error) {
      as_log_debug(ASImageLoadingLog(), "Acquired image from assets library for %@ %@", weakSelf, nextImageIdentifier);
  ...
  // Likewise, if it's a iOS 8 Photo asset, we need to fetch it accordingly.
    ...
    [self _loadPHAssetWithRequest:request identifier:nextImageIdentifier completion:^(UIImage *image, NSError *error) {
      as_log_debug(ASImageLoadingLog(), "Acquired image from Photos for %@ %@", weakSelf, nextImageIdentifier);
  ...
  else // Otherwise, it's a web URL that we can download.
  {
    ...
    [self _fetchImageWithIdentifierFromCache:nextImageIdentifier URL:nextImageURL completion:^(UIImage *imageFromCache) {
      ...
      if (imageFromCache) {
        as_log_debug(ASImageLoadingLog(), "Acquired image from cache for %@ id: %@ img: %@", strongSelf, nextImageIdentifier, imageFromCache);
      ...
      [strongSelf _downloadImageWithIdentifier:nextImageIdentifier URL:nextImageURL completion:^(UIImage *downloadedImage, NSError *error) {
        __typeof__(self) strongSelf = weakSelf;
        if (downloadedImage) {
          as_log_debug(ASImageLoadingLog(), "Acquired image from download for %@ id: %@ img: %@", strongSelf, nextImageIdentifier, downloadedImage);
        } else {
...
- (void)_finishedLoadingImage:(UIImage *)image forIdentifier:(id)imageIdentifier error:(NSError *)error
{
  ...
  if (image || imageIdentifierCount == 0) {
    as_log_debug(ASImageLoadingLog(), "[%p] loaded -> displaying (%@, %@)", self, imageIdentifier, image);
