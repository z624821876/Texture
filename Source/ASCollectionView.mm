...
- (void)beginUpdates
{
  ...
  if (_batchUpdateCount == 0) {
    ...
    _changeSet.rootActivity = as_activity_create("Perform async collection update", OS_ACTIVITY_CURRENT, OS_ACTIVITY_FLAG_DEFAULT);
...
{
  as_activity_scope(as_activity_create("Batch fetch for collection node", OS_ACTIVITY_CURRENT, OS_ACTIVITY_FLAG_DEFAULT));
  [_batchContext beginBatchFetching];
  if (_asyncDelegateFlags.collectionNodeWillBeginBatchFetch) {
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
      GET_COLLECTIONNODE_OR_RETURN(collectionNode, (void)0);
      as_log_debug(ASCollectionLog(), "Beginning batch fetch for %@", collectionNode);
...
- (void)rangeController:(ASRangeController *)rangeController didUpdateWithChangeSet:(_ASHierarchyChangeSet *)changeSet
{
  ...
  ASPerformBlockWithoutAnimation(!changeSet.animated, ^{
    ...
    if (changeSet.includesReloadData) {
      ...
      os_activity_scope(changeSet.rootActivity);
      [_rangeController updateIfNeeded];
    }
  });
}
